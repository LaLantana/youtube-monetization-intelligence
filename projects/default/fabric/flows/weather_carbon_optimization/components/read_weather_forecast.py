...
def _request_json(url: str) -> dict[str, Any]:
    ...
    raise RuntimeError(f"Failed to fetch forecast weather data after retries: {last_error}")
...
def read_weather_forecast(context: ComponentExecutionContext) -> pd.DataFrame:
    ...
    for location in COORDINATES:
        url = _build_forecast_url(location["latitude"], location["longitude"])
        payload = _request_json(url)
        hourly_df = pd.DataFrame(payload["hourly"])
        hourly_df["forecast_timestamp"] = pd.to_datetime(hourly_df["time"], utc=True)
        ...
        log(f"Fetched {len(hourly_df)} forecast weather rows for {location['city']}")
