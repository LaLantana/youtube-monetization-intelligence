WITH monthly_category_metrics AS (
    SELECT
        ...
        AVG(CASE WHEN monetization_quadrant = 'premium' THEN 1.0 ELSE 0.0 END) AS premium_quadrant_share,
    ...
    SELECT
        *,
        ROW_NUMBER() OVER (
            ...
            ORDER BY premium_quadrant_share DESC, avg_recency_weighted_estimated_revenue DESC, youtube_category_name
        ) AS premium_category_rank
...
FROM ranked
WHERE premium_category_rank <= 10
